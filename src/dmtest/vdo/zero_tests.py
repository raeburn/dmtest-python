...
zeros consumes no physical storage (both raw block device and filesystem operations).
...
from dmtest.vdo.utils import BLOCK_SIZE, standard_vdo, mounted_fs
...
def t_dedupe(fix) -> None:
    ...
    with standard_vdo(fix) as vdo:
        ...
        process.run(f"dd if=/dev/zero of={vdo.path} bs={BLOCK_SIZE} count={block_count}")
        process.run("sync")
        ...
        process.run(f"dd if={vdo.path} of=/dev/null bs={BLOCK_SIZE} count={block_count}")
        process.run("sync")
