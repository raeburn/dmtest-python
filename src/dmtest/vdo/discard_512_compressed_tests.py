...
def generate_compressible_data(output_path: str, total_bytes: int, compressibility: float):
    ...
    Args:
        ...
        compressibility: Fraction of data that should be compressible (0.0 to 1.0)
    """
    log.info(f"Generating {total_bytes} bytes of {compressibility*100}% compressible data")

    # Calculate how many bytes should be compressible vs random per sector
    ...
    with open(output_path, 'wb') as f:
        for _ in range(total_sectors):
            # Write compressible portion (0xFF bytes)
            f.write(b'\xFF' * compressible_bytes_per_sector)
            # Write random portion
            f.write(os.urandom(random_bytes_per_sector))


def t_discard_512_compressed(fix) -> None:
    """Test block discard with 512-byte logical block size emulation on compressed data.

    Validates that VDO correctly handles TRIM/discard operations at 512-byte
    granularity on compressed data (90% compressibility), while maintaining 4KB
    internal block management. Tests all possible sector alignments and verifies
    that logical block accounting correctly tracks fully vs. partially discarded
    blocks when data is stored in compressed form.
    """
    log.info("Creating VDO device with 512-byte emulation and compression enabled")
    with standard_vdo(fix, slab_bits=17, compression="on", **{"512_mode": 512}) as vdo:
        log.info(f"VDO device created: {vdo.path}")

        # Construct the discard pattern
        log.info("Constructing discard extent list")
        ...
        # Generate compressible test data file (90% compressible)
        total_bytes = total_blocks * BLOCK_SIZE
        test_data_path = f"/tmp/discard512compressed_test_{process.run('date +%s')[1].strip()}"

        generate_compressible_data(test_data_path, total_bytes, compressibility=0.90)

        device_copy_path = None
        try:
            # Write test data to VDO device using direct I/O
            ...
            # Check initial logical blocks used
            ...
            # Check logical blocks used after discard
            vdo_stats = stats.vdo_stats(vdo)
            ...
            # Verify data integrity by reading back and comparing
            # We need to zero out the discarded regions in the test data for comparison
            ...
            # Read back from VDO device
            device_copy_path = f"/tmp/discard512compressed_copy_{process.run('date +%s')[1].strip()}"
