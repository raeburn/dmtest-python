...
def t_discard_512(fix) -> None:
    ...
    granularity while maintaining 4KB internal block management. Tests all
    possible sector alignments and verifies that logical block accounting
    correctly tracks fully vs. partially discarded blocks.
    """
    log.info("Creating VDO device with 512-byte emulation")
    with standard_vdo(fix, slab_bits=17, **{"512_mode": 512}) as vdo:
        ...
        test_data_path = f"/tmp/discard512_test_{process.run('date +%s')[1].strip()}"
        ...
        try:
            ...
            device_copy_path = f"/tmp/discard512_copy_{process.run('date +%s')[1].strip()}"
